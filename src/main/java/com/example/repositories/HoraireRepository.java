...
public interface HoraireRepository extends JpaRepository<Horaire, Long> {
    ...
    public List<Horaire> findBetween(@Param("from") @DateTimeFormat(iso = ISO.DATE_TIME) LocalDateTime start, @Param("to") @DateTimeFormat(iso = ISO.DATE_TIME) LocalDateTime end);

    @Query("select h from Horaire h where h.startDate = :start and h.endDate = :end")
    public Horaire findDistinctByStartDateAndEndDate(@Param("start") @DateTimeFormat(iso = ISO.DATE_TIME) LocalDateTime start, @Param("end") @DateTimeFormat(iso = ISO.DATE_TIME) LocalDateTime end);
}
